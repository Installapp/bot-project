...
const banCommand = {
    ...
    async execute(interaction) {
        ...
        // Prevent banning the protected owner user ID
        if (user.id === '995631414907244646') {
            return interaction.reply({ content: '❌ امکان بن کردن مالک سرور وجود ندارد.', ephemeral: true });
        }
...
const kickCommand = {
    ...
    async execute(interaction) {
        ...
        // Prevent kicking the protected owner user ID
        if (user.id === '995631414907244646') {
            return interaction.reply({ content: '❌ امکان کیک کردن مالک سرور وجود ندارد.', ephemeral: true });
        }
